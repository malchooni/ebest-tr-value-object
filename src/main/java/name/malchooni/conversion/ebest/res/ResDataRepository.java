...
public class ResDataRepository {
    ...
    public void putResFileData(String resName, ResFileData resFIleData) throws MessageException {
        switch (resFIleData.getResType()) {
            case ResFileData.QUERY -> this.resQuery.put(resName, resFIleData);
            case ResFileData.REAL -> this.resReal.put(resName, resFIleData);
            default -> throw new MessageException("invalid res type");
        }
    }
    ...
    public Map<String, ResFileData> getResMap(String type) {
        return switch (type) {
            case ResFileData.QUERY -> this.resQuery;
            case ResFileData.REAL -> this.resReal;
            default -> null;
        };
    }
